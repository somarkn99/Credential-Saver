    ...
    if (request.action === "fill_credentials" && request.data) {
      ...
      const fillInput = (selectors, value) => {
        ...
        selectors.forEach((selector) => {
          ...
          console.log(
            `Found ${inputs.length} input(s) for selector: ${selector}`
          );
          if (inputs.length > 0) {
            inputs.forEach((input) => {
              if (input) {
                try {
                  console.log(
                    `Setting value for input with selector: ${selector}`
                  );
                  input.value = value;
                  ...
                  console.error(
                    `Error setting value for selector ${selector}:`,
                    error
                  );
                }
              } else {
      ...
      const passwordSelectors = [
        ...
        "input[class*=Password]",
        // "input[name=31password]", // This throw an error 'input[name=31password]' is not a valid selector'
      ];
